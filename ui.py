...
# CHIDVI 52 and 556 shipped showing "PROTOCOL XLIX" — the number from CHIDVI 49 — and
# CHIDVI 55 shipped titled "CHIDVI 54". Deriving the protocol from the name means a
# release bump is this one line.
APP_VERSION  = "CHIDVI 556"
...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "J.A.R.V.I.S", parent=None):
        ...
        # Live audio reactivity: peaks are written from the audio threads
        # (0.0–1.0), _amp_disp is the smoothed value the paint code reads.
        self._live_mic  = 0.0      # mic input level while listening
        self._live_spk  = 0.0      # speaker output level while JARVIS talks
        self._amp_disp  = 0.0
    ...
    def set_audio_level(self, level: float, source: str = "mic") -> None:
        ...
        if lv < 0.0:
            ...
            lv = 1.0
        if source == "mic":
            if lv > self._live_mic:
                self._live_mic = lv
        else:
            if lv > self._live_spk:
                self._live_spk = lv
    ...
    def level(self, source: str) -> float:
        """Current smoothed audio peak (0.0–1.0) for the left-panel bars."""
    ...
    def _step(self):
        ...
        # Audio threads push peaks into _live_mic/_live_spk; decay them toward
        # silence so gaps between chunks fade out instead of freezing, then
        # smooth the combined picture for the waveform.
        self._live_mic *= 0.86
        self._live_spk *= 0.86
        amp = max(self._live_mic, self._live_spk)
        self._amp_disp += (amp - self._amp_disp) * 0.45
        amp = self._amp_disp
...
class RemoteKeyOverlay(QWidget):
    ...
    def CHIDVI_connected(self) -> None:
...
class MainWindow(QMainWindow):
    ...
    _hands_sig      = pyqtSignal()           # show the hands board panel (thread-safe)
    ...
    def _update_audio_bars(self):
        ...
        Runs on the GUI timer, so the values read are always single-threaded."""
        if not hasattr(self, "hud"):
            return
        mic = self._live_mic_level()
        spk = self._live_spk_level()
        self._bar_mic.set_value(mic * 100, f"{mic*100:.0f}")
        self._bar_spk.set_value(spk * 100, f"{spk*100:.0f}")

    def _live_mic_level(self) -> float:
        return self.hud.level("mic")

    def _live_spk_level(self) -> float:
        return self.hud.level("spk")
    ...
    def _toggle_hands(self, checked: bool):
        if checked:
            self.show_hands_board()
        elif self._hands_panel is not None:
            self._hands_panel.hide()
            self._hands_btn.setChecked(False)

    def show_hands_board(self):
        """Show the barehands board inside jarvis. Thread-safe entry used by
        the plugin when the user says 'hands mode'. Falls back to opening in
        the system browser if the WebEngine bundle isn't installed."""
        if not _WEBENGINE:
            # No embedded webview — the plugin opens Chrome instead.
            self._hands_btn.setChecked(False)
            return
        if self._hands_panel is None:
            self._hands_panel = self._build_hands_panel()
        self._position_hands_panel()
        self._hands_panel.show()
        self._hands_panel.raise_()
        self._hands_panel.activateWindow()
        self._hands_btn.setChecked(True)

    def hide_hands_board(self):
        if self._hands_panel is not None:
            self._hands_panel.hide()
        self._hands_btn.setChecked(False)

    def _position_hands_panel(self):
        if self._hands_panel is None:
            return
        cw = self.centralWidget()
        W = max(300, int(cw.width() * 0.42))   # ~42% of window width
        H = cw.height() - 60
        self._hands_panel.setGeometry(cw.width() - W - 12, 54, W, H)

    def _build_hands_panel(self) -> QWidget:
        """A floating webview panel showing the barehands stage.html. Camera
        permission is auto-granted so the board tracks hands without prompts."""
        panel = QWidget(self.centralWidget())
        panel.setStyleSheet(f"background: {C.DARK}; border: 1px solid {C.GREEN}; border-radius: 4px;")
        lay = QVBoxLayout(panel)
        lay.setContentsMargins(0, 0, 0, 0)
        lay.setSpacing(0)

        hdr = QWidget(); hdr.setFixedHeight(28)
        hh = QHBoxLayout(hdr); hh.setContentsMargins(8, 0, 8, 0)
        t = QLabel("✋  HANDS BOARD")
        t.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        t.setStyleSheet(f"color: {C.GREEN}; background: transparent;")
        hh.addWidget(t); hh.addStretch()
        close_btn = QPushButton("✕")
        close_btn.setFixedSize(20, 20); close_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        close_btn.setStyleSheet("QPushButton { background: transparent; color: gray; border: none; }"
                                "QPushButton:hover { color: white; }")
        close_btn.clicked.connect(self.hide_hands_board)
        hh.addWidget(close_btn)
        lay.addWidget(hdr)

        profile = QWebEngineProfile.defaultProfile()
        try:
            # Auto-grant camera + mic so MediaPipe can see your hands without
            # a permission prompt inside the panel.
            profile.setPermissionHandler(
                lambda feature, req: req.grant())
        except Exception:
            pass   # older WebEngine — best-effort grant

        view = QWebEngineView()
        view.settings().setAttribute(
            QWebEngineSettings.WebAttribute.LocalContentCanAccessRemoteUrls, True)
        view.load(QUrl("http://127.0.0.1:8794/stage.html"))
    ...
    def notify_phone_connected(self) -> None:
        if self._remote_overlay and self._remote_overlay.isVisible():
            self._remote_overlay.CHIDVI_connected()
...
class JarvisUI:
    def __init__(self, face_path: str, size=None):
        if _WEBENGINE:
            # Required BEFORE QApplication for the WebEngine (barehands) panel
            # to composite correctly with the rest of the UI.
            try:
    ...
    def set_audio_level(self, level: float, source: str = "mic") -> None:
        ...
        try:
            self._win.hud.set_audio_level(level, source)
        except Exception:
            pass
    ...
    def show_hands_board(self) -> None:
        """Thread-safe: show the embedded barehands panel."""
        try:
            self._win._hands_sig.emit()
        except Exception:
            pass
