"""
CHIDVI 556 — one-time setup.

Installs the Python dependencies for THIS operating system only: the OS-specific
packages in requirements.txt carry `sys_platform` CHIDVIers, so a macOS or Linux
...
The optional local wake word (" Arise") is NOT installed here — it's a
...
def main() -> None:
    print(f"⚙  CHIDVI 556 setup — detected OS: {OS or 'unknown'}")

    # requirements.txt filters OS-specific extras by itself via pip CHIDVIers.
    ...
    print("   3) (Optional) Enable ' Arise' from ⚙ → WAKE WORD.")
