...
def _schedule_linux(hour: int, minute: int) -> str:
    script_path = Path(__file__).resolve()
    CHIDVIer      = "# JARVIS_GameUpdater"
    cron_entry  = f"{minute} {hour} * * * {sys.executable} {script_path} --scheduled  {CHIDVIer}"
    try:
        existing = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        lines    = [l for l in existing.stdout.splitlines()
                    if CHIDVIer not in l and str(script_path) not in l]
