"""Bounded autonomous task orchestration for CHIDVI-556."""
...
class AutonomousOrchestrator:
    ...
    def declarations(self) -> list[dict[str, Any]]:
        return [{"name":t.name,"description":f"[{t.provider}] {t.description}","parameters":t.parameters} for t in self.tools.values()]
    ...
    def run(self, objective: str, *, confirm: Callable[[ToolSpec, dict[str, Any]], bool] | None = None) -> dict[str, Any]:
        ...
        for step in range(1, self.max_steps + 1):
            ...
            if not isinstance(decision, dict): return self._finish(False, "Planner returned an invalid decision.")
            if decision.get("done") is True: return self._finish(True, decision.get("summary", "Task completed."))
            name, args = decision.get("tool"), decision.get("arguments") or {}
            tool = self.tools.get(name)
            if tool is None:
                self.history.append(StepResult(step, str(name), False, "Unknown tool.")); continue
    ...
    def _finish(self, ok: bool, summary: str) -> dict[str, Any]:
        return {"ok":ok,"summary":summary,"steps":[{"step":r.step,"tool":r.tool,"ok":r.ok,"output":r.output} for r in self.history]}
